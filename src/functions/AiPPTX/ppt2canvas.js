...
function Ppt2Canvas(_canvas, imageCrossOrigin) {
    ...
    /**
     * 计算文本高度，使用与 drawTextP 一致的换行逻辑
     * 关键：按 splitWords 分割的片段测量宽度，与 SVG 的 getComputedTextLength 保持一致
     */
    function calcTextHeight(obj) {
        ...
        for (let i = 0; i < obj.children.length; i++) {
            ...
            for (let j = 0; j < p.children.length; j++) {
                ...
                ctx.font = fonts.join(' ')
                // 使用与 drawTextP 一致的 splitWords 逻辑
                let words = splitWords(text)
                for (let w = 0; w < words.length; w++) {
                    let wordWidth = ctx.measureText(words[w].text).width
                    if (wordWrap && currentLineWidth + wordWidth > maxWidth + fontSize / 4 && currentLineWidth > 0) {
                        lineCount++
            ...
            totalTextHeight += lineCount * lineSpacing * maxFontSize
            if (underlined) {
                totalTextHeight += 1
            }
        }
        ...
        for (let i = 0; i < p.children.length; i++) {
            ...
            if (isVertical) {
                ...
                for (let j = 0; j < texts.length; j++) {
                    ...
                    // 按片段测量宽度，与SVG的getComputedTextLength保持一致
                    let itemWidth = ctx.measureText(item.text).width
                    // 如果当前片段超出边界，先换行
                    if (x + itemWidth > endX + fontSize / 4 && x > _x) {
                        drawLine(x - _x)
                        x = _x
                        y += lineSpacing * fontSize
                    }
                    // 如果单个片段就超出边界（超长单词），则逐字符绘制
                    if (x + itemWidth > endX + fontSize / 4) {
                        for (let s = 0; s < item.text.length; s++) {
                            let text = item.text[s]
                            let textWidth = ctx.measureText(text).width
                            if (x + textWidth > endX + fontSize / 4 && !isSymbol(text) && x > _x) {
                                drawLine(x - _x)
                                x = _x
                                y += lineSpacing * fontSize
                            }
    ...
    */
    ...
    /**
     * 将当前 PPT 导出为图片 ZIP
     * 使用 JSZip 将所有页面打包为单个 ZIP 文件下载
     */
    this.downloadAsImages = async (pptxObj, fileName = 'pptx') => {
        // 动态加载 JSZip
        ...
        if (!JSZip) {
            try {
                ...
                console.log('JSZip 加载失败，尝试 CDN 加载')
            }
        }
        if (!JSZip) {
            // 从 CDN 加载
        ...
        for (let i = 0; i < pptxObj.pages.length; i++) {
            // 创建临时 canvas 导出高分辨率图片
            const exportCanvas = document.createElement('canvas')
            exportCanvas.width = pptxObj.width * 2
            exportCanvas.height = pptxObj.height * 2
        ...
        // 添加说明文件
        zip.file('README.txt', `PPT 导出文件
主题：${fileName}
页数：${pptxObj.pages.length}
生成时间：${new Date().toLocaleString()}

说明：本 ZIP 包含 PPT 每页的高清 PNG 图片，按 slide_01.png, slide_02.png 顺序排列。`)
